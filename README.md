...
> ##### 같은 이름의 함수를 매개변수의 형이나 개수를 다르게 하여 중복 정의하는 것을 의미한다
> ##### 객체 지향 프로그래밍의 특징 중 하나인 다형성(polymorphism)의 구현이다
> ##### 함수 시그니처(Function signature)
>> ##### 함수의 원형에 명시되는 매개변수 리스트를 가리킨다
>> ##### 만약 두 함수가 매개변수의 개수와 그 타입이 모두 같다면, 이 두 함수의 시그니처는 같다
>> ##### 즉, 함수의 오버로딩은 서로 다른 시그니처를 갖는 여러 함수를 같은 이름으로 정의하는 것이다
> ##### 컴파일러는 자동으로 같은 함수 시그니처를 를 가지는 함수의 원형을 찾아 호출한다
> ##### - 함수를 호출할 때 지정한 인수의 개수와 형이 정의되어 있는 어떤 함수와도 맞지 않을 경우 컴파일 에러가 발생
> ##### 매개변수가 같고 반환 값의 형만 다른 경우 오버로딩 할 수 없다
> ##### 모호한 호출인 경우 컴파일 에러가 발생한다
> ```c++
> void Display(int x, int y); // 1
> void Display(double x, double y); // 2
>
> Display(4.2, 3);  // 1, 2 모두 호출할 수 있으므로 컴파일 에러 발생
> ```
